...
from .engine import Firewall, InspectionResult, _decision_applies, _decisions_blocked
...
def sanitize_observability(payload: Any, firewall: Firewall) -> InspectionResult:
    ...
    if applicable:
        ...
        if root_decisions:
            transformed = transform_scalar_value(transformed, root_decisions)
        else:
            transformed = apply_decisions(transformed, applicable)
