...
You can also extract `lsdf-0.4.1-source.zip` from the [release downloads](https://github.com/brntech/lsdf/releases/tag/v0.4.1) and open its folder. These commands build images locally; no separate image pull is needed. The first build needs internet access and may take several minutes. For a prebuilt gateway without a source checkout, use the [installation guide](docs/installation.md#prebuilt-release-image).
...
- The v0.4.1 gateway inspects response metadata and parsed tool-argument keys. Enforcing findings withhold these fields instead of rewriting identifiers or argument names; see the [security model](docs/security-model.md#detector-posture).
- v0.4.1 fixes false blocks on recognized vLLM response metadata and coding-tool permission-refusal feedback. The exact compatibility boundaries are documented in the [security model](docs/security-model.md).
...
To verify caller protection, use the current source checkout with the dependency-free demo upstream. The published v0.4.1 image includes this control; this recipe is deliberately source-built and cannot incur provider charges:
