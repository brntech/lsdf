...
| Non-streaming responses | Supported in v0.4.1 | Request preflight, content transforms, and metadata inspection without identity rewriting. Unsupported non-SSE response bodies are withheld with HTTP 502. See [metadata coverage](security-model.md). |
| Streaming response metadata | Inspected in v0.4.1 | JSON metadata and SSE envelope values are preflighted. Unsafe metadata withholds pending content. JSON IDs must remain stable; tool IDs/names must arrive as complete initial values. See [streaming compatibility](streaming-compatibility.md#server-sent-events-node-deno-eventsource-api). |
...
| Streamed tool-call arguments | Supported | Arguments are assembled before release. The v0.4.1 gateway scans parsed JSON keys and values; enforcing key findings withhold the payload without renaming keys. The v0.4.1 image includes key inspection and metadata enforcement. |
...
| Client authentication | Supported in v0.4.1 release/runtime | Set `LSDF_CLIENT_TOKEN` independently from `LSDF_MANAGEMENT_TOKEN`; v0.4.1 includes this control. |
| Request and stream limits | Supported in v0.4.1 release/runtime | `LSDF_MAX_REQUEST_BYTES`, `LSDF_MAX_CONCURRENT_REQUESTS`, client/upstream timeouts, maximum stream duration, and stream holdback are reported by authenticated health; no sustained-load certification is implied. |
...
Upgrade note: the historical v0.3.2 release predates the client-token, request-limit, response-metadata and tool-argument key controls. v0.4.1 includes them.

vLLM metadata: v0.4.1 recognizes the root `prompt_token_ids` field name when `object` is `chat.completion` or `chat.completion.chunk`, `choices` is a list and no `messages` field is present. Its values and unknown keys remain inspected; other response shapes remain fully inspected. The earlier v0.4.0 images can reject this field name as a secret; [upgrade to v0.4.1](installation.md#upgrading-an-existing-deployment) if affected. Token IDs can encode recoverable prompt text. LSDF inspects their scalar values but does not decode them, so sensitive text encoded as token IDs is outside its detection coverage.

v0.4.1 also recognizes a bounded subset of generated vLLM `system_fingerprint` values at the same response root. Only the entropy heuristic accepts the constrained version/parallelism/hash grammar described in the [security model](security-model.md); other detectors still inspect the value. Unrecognized custom or malformed fingerprints remain inspected normally. Both image variants include these corrections.

Tool refusal feedback: v0.4.1 avoids entropy false positives on recognized permission-rule labels and constrained POSIX tool-output cache globs within a bounded permission-refusal tool result. Other fields and detector families remain active. This does not grant the client permission to execute a tool; see the [security model](security-model.md).
