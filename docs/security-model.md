...
v0.4.1 applies an additional entropy-only compatibility rule to a string at root `system_fingerprint` under the same envelope conditions. The complete value must match LSDF's constrained approximation of [vLLM's generated fingerprint](https://github.com/vllm-project/vllm/blob/main/vllm/entrypoints/serve/utils/fingerprint.py): `vllm-`, a numeric three-part version or `dev`, supported prerelease/post/dev and Git/CUDA version suffixes, ordered nontrivial `tp`/`pp`/`dp` and `ep` settings when present, and an eight-character lowercase hexadecimal configuration hash or `nohash`. Unknown local-version labels, other locations and non-string values get no such exception. Regex and other detector families still inspect every value. A custom fingerprint can imitate this grammar; matching it does not authenticate the backend or rule out encoded data. Both v0.4.1 image variants include this correction.

The entropy detector recognizes two narrowly scoped permission-rule value forms in `input.tool_results`: the exact `external_directory` label and a constrained absolute POSIX cache glob ending `/opencode/tool-output/*`. The tool text must be a recognized permission-refusal message followed by a complete JSON rules array: at most 32 rules, a 16,384-character limit on the entire tool-result string, unique `permission`/`pattern`/`action` string fields, and an `allow`, `deny` or `ask` action. Cache parent components must be short lowercase letter/dot/hyphen names without traversal or credential markers; both individual components and their concatenation must pass the normal secret-like entropy check. Only those literal spans skip entropy; other spans and all other detectors still run. Malformed, unknown, escaped or unsupported forms receive normal inspection. This format can be imitated and is not trusted provenance, proof against encoded data, or permission to execute a tool.
